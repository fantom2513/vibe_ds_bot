...
def test_skipped_tests_are_excluded_from_passed_and_failed(tmp_path):
    path = _write(tmp_path, """
        <testsuites>
          <testsuite name="pytest" tests="3" failures="0" errors="0" skipped="1" time="0.3">
            <testcase classname="tests.test_a" name="test_one" time="0.1"/>
...
def test_missing_file_returns_readable_summary_without_raising(tmp_path):
    path = tmp_path / "does-not-exist.xml"

    summary = summarize(path, label="py3.12")

    assert "py3.12" in summary
    assert "е удалось прочитать" in summary


def test_empty_file_returns_readable_summary_without_raising(tmp_path):
    path = tmp_path / "results.xml"
...
def test_truncated_xml_returns_readable_summary_without_raising(tmp_path):
    path = tmp_path / "results.xml"
    path.write_text(
        textwrap.dedent("""
            <testsuites>
              <testsuite name="pytest" tests="2" failures="0" errors="0" skipped="0" time="1.5">
                <testcase classname="tests.test_a" name="test_one" time="1.0"/>
                <testcase classname="tests.test_a" name="test_
        """).strip(),
        encoding="utf-8",
    )
