  ...
  return (
    <Box sx={{ border: '1px solid var(--color-border)', borderRadius: 2, p: 1.75 }}>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1 }}>
        ...
        {rule.target_list ? (
          <StatusBadge tone={rule.target_list === 'whitelist' ? 'success' : 'danger'}>
            {targetListLabel(rule.target_list)}
          </StatusBadge>
        ) : (
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      channel_ids: form.channel_ids
        ? form.channel_ids.split(',').map(s => s.trim()).filter(Boolean).map(Number)
        : [],
              ...
              {rules.map(r => {
                ...
                return (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      {r.target_list ? (
                        <StatusBadge tone={r.target_list === 'whitelist' ? 'success' : 'danger'}>
                          {targetListLabel(r.target_list)}
                        </StatusBadge>
                      ) : (
                        <Typography sx={{ color: 'text.disabled', fontSize: '0.72rem' }}>Все участники</Typography>
                      )}
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Switch
                          ...
                          inputProps={{ 'aria-label': `Переключить: ${r.name}` }}
                        />
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        <Tooltip title={`Редактировать: ${r.name}`}>
                          <IconButton size="small" aria-label={`Редактировать: ${r.name}`} onClick={() => openEdit(r)}>
                            <EditOutlined sx={{ fontSize: 15 }} />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={`Удалить: ${r.name}`}>
                          <IconButton size="small" color="error" aria-label={`Удалить: ${r.name}`} onClick={() => setDeleteTarget(r)} disabled={pending}>
                            <DeleteOutlined sx={{ fontSize: 15 }} />
                          </IconButton>
                        </Tooltip>
                      </Box>
